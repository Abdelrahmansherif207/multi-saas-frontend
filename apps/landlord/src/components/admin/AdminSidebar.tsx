...
export function SidebarContent({ onClose, collapsed = false }: { onClose?: () => void, collapsed?: boolean }) {
    const pathname = usePathname();
    const t = useTranslations("Admin.RoleManage.menu");
    const tSidebar = useTranslations("Sidebar"); // Assuming there's a general sidebar translation or I should add keys for other items too?
    // Actually, existing items are not localized in my previous view of sidebar.
    // The user didn't ask to localize *everything*, but "Localize Admin Role Management Pages".
    // But consistency is key.
    // Let's check if there are keys for other items. I didn't see them in JSON view.
    // I will primarily focus on "Admin Role Manage" items as requested.
    ...
    // Initialize with expanding sections that contain the current path
    ...
    return (
        <div className="flex flex-col h-full bg-sidebar/50 md:bg-transparent">
            {/* Logo Area */}
            <div className={cn("h-20 flex items-center transition-all duration-300", collapsed ? "justify-center px-0" : "px-6 md:px-8")}>
            ...
            <div className={cn("flex-1 overflow-y-auto py-6 space-y-3", collapsed ? "px-2" : "px-4")}>
                {sidebarItems.map((item) => {
                    const isExpanded = expandedItems.includes(item.title);
                    const isActiveParent = item.subItems?.some(sub => sub.href === pathname);
                            ...
                            <div key={item.title} className="space-y-3">
                                ...
                                >
                                    <item.icon className={cn(
                                        ...
                                        isActiveParent ? "text-white dark:text-black" : "text-muted-foreground group-hover:text-foreground"
                                    )} />
                                    {!collapsed && (
                                        <>
                                            ...
                                            return (
                                                ...
                                                >
                                                    <ArrowRight className={cn(
                                                        ...
                                                        isSubActive ? "opacity-100 translate-x-0.5" : "opacity-40 group-hover/sub:translate-x-0.5 group-hover/sub:opacity-100"
                                                    )} />
                                                    <span>{sub.title}</span>
                                                </Link>
                                            );
